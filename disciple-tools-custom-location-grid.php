<?php
/**
 * Plugin Name: Disciple.Tools - Custom Location Grid
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-custom-location-grid
 * Description: Disciple.Tools - Custom Location Grid is intended to help developers and integrator jumpstart their extension of the Disciple.Tools system.
 * Text Domain: disciple-tools-custom-location-grid
 * Domain Path: /languages
 * Version:  0.1
 * Author URI: https://github.com/DiscipleTools
 * GitHub Plugin URI: https://github.com/DiscipleTools/disciple-tools-custom-location-grid
...
/**
 * Gets the instance of the `DT_Custom_Location_Grid` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_custom_location_grid() {
    $disciple_tools_custom_location_grid_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_custom_location_grid_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'disciple_tools_custom_location_grid_hook_admin_notice' );
    ...
    return DT_Custom_Location_Grid::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_custom_location_grid', 20 );
...
class DT_Custom_Location_Grid {
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-custom-location-grid' );
    }
    ...
    public function i18n() {
        $domain = 'disciple-tools-custom-location-grid';
    ...
    public function __toString() {
        return 'disciple-tools-custom-location-grid';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "disciple_tools_custom_location_grid::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'DT_Custom_Location_Grid', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'DT_Custom_Location_Grid', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_custom_location_grid_hook_admin_notice' ) ) {
    function disciple_tools_custom_location_grid_hook_admin_notice() {
        global $disciple_tools_custom_location_grid_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple.Tools - Custom Location Grid' plugin requires 'Disciple.Tools' theme to work. Please activate 'Disciple.Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_custom_location_grid_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-custom-location-grid', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-custom-location-grid is-dismissible" data-notice="disciple-tools-custom-location-grid">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-custom-location-grid .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-custom-location-grid',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-custom-location-grid/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-custom-location-grid/master/version-control.json',
//                __FILE__,
